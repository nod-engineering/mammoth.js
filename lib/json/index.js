var getReadableElement = require('./utils').getReadableElement;
var parentEl = {};
var assetsArray = [];
...
const LISTS_TAGS = ['bulletList', 'numberList'];
function write(writer, nodes) {
    nodes.forEach(function(node) {
        var tagName = getReadableElement(node.tag.tagName);
        if (parentEl && LISTS_TAGS.includes(parentEl.type) && LISTS_TAGS.includes(tagName)) {
