...
function simpleWriter() {
    function element(node, assetsArray, parentEl) {
        ...
        } else if(['bulletList', 'numberList'].includes(parenType)) {
            if(tagName === 'listItem') {
                parentEl.nodes.push({
                    data: {},
                    type: 'listItem',
                ...
                const listItemIndex = parentEl.nodes.length - 1 || 0;

                parentEl.nodes[listItemIndex].nodes.push({
                    data: {},
                    type: 'listItemChild',
                    nodes: [{
                        text: '',
                ...
                const listItemIndex = parentEl.nodes.length - 1 || 0;
                const listItemChildIndex = parentEl.nodes[listItemIndex].nodes.length -1 || 0;
                parentEl.nodes[listItemIndex].nodes[listItemChildIndex].nodes.push({
                    data: {
                        href: node.tag.attributes.href
                    },
                    type: tagName,
    ...
    function text(value, parentEl) {
        ...
        if(_.has(parentEl, 'nodes')) {
            ...
            if(isTable) {
                ...
                } else if(pEl && !isEmpty(get(pEl.nodes[pElIndex], 'data.href'))) {
                    const pTextIndex = pEl.nodes[pElIndex].nodes.length === 0 ? 0 : pEl.nodes[pElIndex].nodes.length - 1;
                    if(pEl.nodes[pElIndex].nodes[pTextIndex].text === '') {
                        pEl.nodes[pElIndex].nodes[pTextIndex].text = value;
                    } else {
                        pEl.nodes[pElIndex].nodes.push(textObj);
                    }
                ...
                if(isEmpty(get(listEl, 'nodes'))) {
                    ...
                    const listItemChildIndex = listEl.nodes.length -1 || 0;
                    const lChildEl = listEl.nodes[listItemChildIndex];
                    const lChildNodeIndex = lChildEl.nodes.length - 1 || 0;
                    const childNodeEl = lChildEl.nodes[lChildNodeIndex];
                    if(childNodeEl && _.has(childNodeEl, 'marks') && childNodeEl.text === '') {
                        childNodeEl.text = value;
                    } else if(get(childNodeEl, 'data.href') && childNodeEl.nodes[0].text === '') {
                        childNodeEl.nodes[0].text = value;
                    } else {
                        lChildEl.nodes.push(textObj);
                    }
